...
class TreeMap(LinkedBinaryTree, MapBase):
    ...
    def after(self, p):
        ...
        """
        self._validate(p)  # symmetric to before(p)
        if self.right(p) is not None:
    ...
    def find_ge(self, k):
        """Return (key, value) pair with least key >= k. Return None if none."""
        if self.is_empty():
            return None
    ...
    def __setitem__(self, k, v):
        ...
        self.rebalance_insert(leaf)                   # hook for balanced tree subclasses
    ...
    def delete(self, p):
        ...
        parent = self.parent(p)
        super()._delete(p)  # inherited from LinkedBinaryTree (removes node with ≤1 child)
    ...
    def _relink(self, parent, child, make_left_child):
        """Relink parent node with child node (child may be None). Uses underscored internals."""
    ...
    def _rotate(self, p):
        """Rotate Position p above its parent. Uses underscored internals."""
        x = p._node
        y = x._parent                       # must exist
        ...
        # rotate x and y, transferring middle subtree
        if x == y._left:
    ...
    def _restructure(self, x):
        """Perform trinode restructure of Position x with parent/grandparent. Uses underscored internals."""
        y = self.parent(x)
        z = self.parent(y)
...
class AVLTreeMap(TreeMap):
    ...
    class _Node(TreeMap._Node):
        ...
        __slots__ = TreeMap._Node.__slots__ + ('_height',)
    ...
    def _rebalance(self, p):
        while p is not None:
            ...
            if not self._isbalanced(p):
                ...
                if self.left(p)  is not None: self._recompute_height(self.left(p))
                if self.right(p) is not None: self._recompute_height(self.right(p))
                self._recompute_height(p)
            p = self.parent(p)
